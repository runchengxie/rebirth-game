...
`src/game/scenes.ts` 是场景的唯一契约：`StageSceneId` 描述年度剧情的位图场景（研究室、会议简报室、夜间咖啡馆），`ProceduralSceneId` 描述投委会的 CSS 程序化场景（会议室、数据墙、风险审查、合规桌面）。注册表为位图场景登记来源、生成方式、授权、尺寸、压缩信息，以及构图焦点、灯光与粒子策略。`SceneNode.bg` 只接受已注册编号，未注册场景在类型检查阶段失败。

背景位图按需加载。`stageSceneImageUrl` 只在场景首次出现时解析资源 URL 并全局缓存，`buildSceneView` 暴露下一节点背景供舞台预取。`src/components/PixiStage.tsx` 只在启动时加载当前背景和四位角色的透明立绘，其余背景在切换或预取时补载。舞台负责姿势映射、缩放、底部锚点、色调和阴影，系统要求减少动态效果时不创建漂浮星点。`src/app/useGameController.ts` 会先检测 WebGL，失败时静态舞台从同一注册表按需注入当前场景背景（行内样式），观点交锋亦然，CSS 不再静态引用场景位图。
