...
export function PortfolioChart({ state }: { state: GameState }) {
  ...
  for (let i = 0; i < state.history.length; i++) {
    ...
    bestPath.push(prev * 1.02); // Progressive benchmark: 2% monthly
  }

  const values = [...selectedPath, ...bestPath, 1.5].filter((v) => v > 0);
  ...
  return (
    <svg className="capital-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="组合模拟净值曲线">
      {[0, 1, 2, 3, 4].map((item) => {
      ...
      <text className="chart-label" x={pad.left - 8} y={y(1.5)} textAnchor="end">
        1.500
      </text>
      <line className="chart-target" x1={pad.left} x2={width - pad.right} y1={y(1.5)} y2={y(1.5)} />
